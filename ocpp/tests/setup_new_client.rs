...
pub async fn setup_new_client() -> Result<OCPP1_6Client, Box<dyn std::error::Error + Send + Sync + 'static>> {
    let username = Uuid::new_v4().to_string();
    let client = connect_1_6(&format!("ws://localhost:3000/{}", username), None).await?;
    ...
    let (boot_result, get_result, changeResult) = join3(fut1, fut2, fut3).await;

    let boot_request = boot_result??;
    assert_eq!(boot_request.status, RegistrationStatus::Pending);
    get_result?;
    let change_request = changeResult?;


    let password = String::from_utf8(hex::decode(&change_request.value)?)?;
    client.disconnect().await?;
    sleep(Duration::from_millis(2000)).await;
    let client = connect_1_6(&format!("ws://localhost:3000/{}", username), Some(ConnectOptions {
        username: Some(&username),
        password: Some(&password)
    })).await?;

    let response = client.send_boot_notification(BootNotificationRequest {
        charge_box_serial_number: Some("TEST123123".to_string()),
        charge_point_model: "TEST_MODEL".to_string(),
        charge_point_serial_number: Some("TEST123123".to_string()),
        charge_point_vendor: "TEST_VENDOR".to_string(),
