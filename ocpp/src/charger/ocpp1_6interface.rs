...
impl<'a> Ocpp1_6Interface<'a> {
    ...
    pub async fn send_remote_start_transaction(&self, request: RemoteStartTransactionRequest) -> Result<Result<RemoteStartTransactionResponse, OCPP1_6Error>, Box<dyn std::error::Error + Send + Sync + 'static>> {
