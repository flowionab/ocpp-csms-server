...
impl Charger {
    ...
    pub fn start_transaction(&mut self) -> Result<String, Status> {
        match self.protocol {
            None => {
                Err(Status::failed_precondition("The charger has not picked a ocpp protocol yet"))
            }
            Some(protocol) => {
                match protocol {
                    OcppProtocol::Ocpp1_6 => {
                        self.ocpp1_6().send_remote_start_transaction(RemoteStartTransactionRequest {
                            connector_id: None,
                            id_tag: "".to_string(),
