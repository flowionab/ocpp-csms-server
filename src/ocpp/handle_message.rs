...
pub async fn handle_message(charger: Arc<Mutex<Charger>>, message: Message, protocol: OcppProtocol, sink: Arc<tokio::sync::Mutex<SplitSink<WebSocketStream, Message>>>) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
    if let Text(raw_payload) = message {
        match protocol {
            OcppProtocol::Ocpp1_6 => {
                handle_ocpp_1_6_message(charger, &raw_payload, sink).await?
            }
...
async fn handle_ocpp_1_6_message(charger: Arc<Mutex<Charger>>, raw_payload: &str, sink: Arc<tokio::sync::Mutex<SplitSink<WebSocketStream, Message>>>) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
...
async fn handle_ocpp_1_6_call(charger: Arc<Mutex<Charger>>, raw_payload: &str, sink: Arc<tokio::sync::Mutex<SplitSink<WebSocketStream, Message>>>) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
    ...
    match response {
        Ok(payload) => {
            ...
            lock.send(Message::Text(serde_json::to_string(&(OCPP_CALL_RESULT, message_id, payload))?)).await?;
        }
        Err(error) => {
            warn!(
                charger_id = lock.id.to_string(),
