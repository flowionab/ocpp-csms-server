...
async fn it_should_handle_start_transaction() -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
    let client = setup_new_client().await?;
    
    let result = client.send_start_transaction(StartTransactionRequest {
        connector_id: 1,
