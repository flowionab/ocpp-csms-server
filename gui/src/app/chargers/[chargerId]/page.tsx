...
export default async function Page(props: {
    ...
    return <div className={"container mx-auto"}>
        ...
        {
            charger.outlets.map(outlet => (
                <div key={outlet.id} className={"rounded-lg dark:bg-neutral-800 shadow-md p-4 mb-4 flex flex-col"}>
                    <span className={"text-xl"}>Outlet {outlet.ocppConnectorId} - {outlet.status ?? "Unknown"}</span>
                    <StartTransactionButton chargerId={chargerId} outletId={outlet.id}/>
                    <ChangeOutletAvailabilityButton chargerId={chargerId} outletId={outlet.id} status={outlet.status}/>
                    <span className={"text-xs dark:text-gray-400"}>{outlet.id}</span>
                </div>))
        }
    </div>
}